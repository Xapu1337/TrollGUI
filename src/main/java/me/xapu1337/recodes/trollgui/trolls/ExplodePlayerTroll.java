...
public class ExplodePlayerTroll extends Troll {
    ...
    public TrollMetaData setMetaData() {
        return (new TrollMetaData(XMaterial.TNT, services)
                .setTrollName("explodePlayer")
                .setAttributes(TrollAttributes.POSSIBLE_DESTRUCTION, TrollAttributes.POSSIBLE_DEATH_OR_ITEM_LOSS,
                        TrollAttributes.POSSIBLE_CRASH_OR_FREEZE)
        ...
        victimWorld.createExplosion(getVictim().getLocation(),
                TrollCore.getInstance().getConfig()
                        .getBoolean("menus.troll-menu.items.trolls.explodePlayer.options.explodeRandomness")
                                ? random.nextInt(TrollCore.getInstance().getConfig().getInt(
                                        "menus.troll-menu.items.trolls.explodePlayer.options.explodeRadius") + 1)
                                : TrollCore.getInstance().getConfig().getInt(
                                        "menus.troll-menu.items.trolls.explodePlayer.options.explodeRadius") + 1,
                false);
