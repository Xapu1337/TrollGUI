...
public class ScareTroll extends Troll {
    ...
    public void execute() {
        ...
        for (int i = 0; i < 32; i++) {
            getVictim().playSound(getVictim().getLocation(), Objects.requireNonNull(XSound.ITEM_TOTEM_USE.parseSound()),
                    SoundCategory.MASTER, 1, 1);
            getVictim().playSound(getVictim().getLocation(),
                    Objects.requireNonNull(XSound.ENTITY_GHAST_HURT.parseSound()), SoundCategory.MASTER, 1, 1);
            getVictim().playSound(getVictim().getLocation(),
                    Objects.requireNonNull(XSound.ENTITY_ELDER_GUARDIAN_DEATH.parseSound()), 1, 1);
