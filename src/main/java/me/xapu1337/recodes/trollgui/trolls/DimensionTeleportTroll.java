...
public class DimensionTeleportTroll extends Troll {
    ...
    public TrollMetaData setMetaData() {
        return (
                new TrollMetaData(XMaterial.ENDER_PEARL, services)
        ...
        getCaller().openInventory(
                new MenuSelectionInventory
                        ...
                        .onClick( (caller, clickedItemName) -> {
                            services.messages().setClassPlaceholders(this.getClass(), "WORLD_NAME", clickedItemName);
                            ...
                            if (selectedWorld == null) {
                                ...
                                getCaller().sendMessage(services.messages().$("The world {WORLD_NAME} does not exist. (has it been loaded yet?)"));
                                return;
                            }
                            if (services.utils().teleportTo(selectedWorld, getVictim(), getVictim().getLocation().getX(), getVictim().getLocation().getZ()))
                                getCaller().sendMessage(services.messages().$("{config:messages.teleportedToDimension}").replaceAll("%PLAYER%", getVictim().getDisplayName()).replaceAll("%DIMENSION%", selectedWorld.getName().toUpperCase()));
                            else
                                getCaller().sendMessage(services.messages().$("{config:messages.teleportFailed}"));
                        })
                        .build("test")
                        .getInventory()
        );
